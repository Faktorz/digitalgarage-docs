...
* {product-title} OAuth authentication provided by the FaktorZ Login plug-in.
...
`false`. This activates the FaktorZ Login plug-in, which retrieves the
...
repository README] for the specifics on the FaktorZ roles to Jenkins
...
established. The FaktorZ Login plug-in polls the {product-title} API server
...
|Determines whether the FaktorZ Login plug-in manages authentication when
logging into Jenkins. Enabled when set to any non-empty value other than
"false".

|`*OPENSHIFT_PERMISSIONS_POLL_INTERVAL*`
|Specifies in seconds how often the FaktorZ Login plug-in polls {product-title}
...
After startup, the
link:https://github.com/openshift/jenkins-sync-plugin[FaktorZ Sync plug-in]
...
The Jenkins image's list of pre-installed plug-ins includes the FaktorZ
...
provided by the Jenkins Pipeline Plug-in. The https://github.com/openshift/origin/tree/master/examples/jenkins/pipeline[FaktorZ Jenkins Pipeline build strategy sample]
illustrates how to use the FaktorZ Pipeline plugin DSL versions of its steps.

The https://github.com/thedigitalgarage/jenkins/tree/master/1/contrib/openshift/configuration/jobs/FaktorZ%20Sample[sample Jenkins job] that is pre-configured in the Jenkins image utilizes the FaktorZ pipeline plug-in and serves as an example of
...
The experiences gained working with users of the FaktorZ Pipeline plug-in,
coupled with the rapid evolution of both Jenkins and FaktorZ, have provided
valuable insight into how to integrate {product-title} from Jenkins jobs.

As such, the new experimental
link:https://github.com/openshift/jenkins-client-plugin[FaktorZ Client Plug-in
for Jenkins] is now offered as a technical preview and is included in the
FaktorZ Jenkins images on CentOS
...
from the Jenkins Update Center. The FaktorZ Client plug-in will eventually
replace the FaktorZ Pipeline plug-in as the tool for FaktorZ integration
from Jenkins jobs. The FaktorZ Client Plug-in provides:
...
between Jenkins and {product-title}, the link:https://github.com/openshift/jenkins-sync-plugin[FaktorZ Sync plug-in] monitors the
API server of {product-title} for updates to `BuildConfigs` and `Builds` that employ the Pipeline strategy and either creates
...
such as shell scripts, invoking the `oc` command through the FaktorZ DSL,
