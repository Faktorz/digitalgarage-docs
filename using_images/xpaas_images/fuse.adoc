...
== Overview
Red Hat JBoss Fuse Integration Services provides a set of tools and containerized xPaaS images that enable development, deployment, and management of integration microservices within FaktorZ.
...
* Fuse Management Console is not included as Fuse administration views have been integrated directly within the FaktorZ Web Console.
* An application deployment with Fuse Integration Services consists of an application and all required runtime components packaged inside a Docker image. Applications are not deployed to a runtime as with Fuse, the application image itself is a complete runtime environment deployed and managed through FaktorZ.
* Patching in an FaktorZ environment is different from standalone Fuse since each application image is a complete runtime environment. To apply a patch, the application image is rebuilt and redeployed within FaktorZ. Core FaktorZ management capabilities allow for rolling upgrades and side-by-side deployment to maintain availability of your application during upgrade.
* Provisioning and clustering capabilities provided by Fabric in Fuse have been replaced with equivalent functionality in Kubernetes and FaktorZ. There is no need to create or configure individual child containers as FaktorZ automatically does this for you as part of deploying and scaling your application.
* Messaging services are created and managed using the A-MQ xPaaS images for FaktorZ and not included directly within Fuse. Fuse Integration Services provides an enhanced version of the camel-amq component to allow for seamless connectivity to messaging services in FaktorZ through Kubernetes.
* Live updates to running Karaf instances using the Karaf shell is strongly discouraged as updates will not be preserved if an application container is restarted or scaled up. This is a fundamental tenet of immutable architecture and essential to achieving scalability and flexibility within FaktorZ.
...
You can start using Fuse Integration Services by creating an application and deploying it to FaktorZ using one of the following application development workflows: 

* Fabric8 Maven Workflow
* FaktorZ Source-to-Image (S2I) Workflow
...
. Set the following environment variables to communicate with FaktorZ and a Docker daemon:
...
| KUBERNETES_MASTER | Specifies the URL for contacting the FaktorZ API server | `https://10.1.2.2:8443`

| KUBERNETES_DOMAIN | Domain used for creating routes. Your FaktorZ API server must be mapped to all hosts of this domain. | `openshift.dev`

|===
+

. Login to FaktorZ using CLI and select the project to which to deploy.
...
. Build and push the project to FaktorZ. You can use following maven goals for building and pushing docker images.
...
| docker:push | Pushes the locally built docker image to the global or a local docker registry. This step is optional when developing on a single node FaktorZ cluster.
...
| mvn -Pf8-local-deploy | Comprises of `clean`, `install`, `docker:build`, `fabric8:json`, and `fabric8:apply`. This will create docker and JSON templates and then apply them to FaktorZ.

| mvn -Pf8-deploy: | Comprises of `clean`, `docker:build`, `fabric8:json`, `docker:push`, and `fabric8:apply`. This will create docker and JSON templates, push them to docker registry and apply to FaktorZ.
...
. Login to FaktorZ Web Console. A pod is created for the newly created application. You can view the status of this pod, deployments and services that the application is creating.

==== Authenticating Against a Registry
For multi node FaktorZ setups, the image created must be pushed to the FaktorZ registry. This registry must be reachable from the outside through a route. Authentication against this registry reuses the FaktorZ authentication with `oc login`. Assuming that your FaktorZ registry is exposed as `registry.openshift.dev:80`, the project image can be deployed to the registry with following command:
...
To push changes to the registry, the FaktorZ project must exist and the users of Docker image must be connected to the FaktorZ project. All the examples uses the property `fabric8.dockerUser` as Docker image user which has `fabric8/` as default (note the trailing slash). When this user is used unaltered an FaktorZ project 'fabric8' must exist. This can be created with 'oc new-project fabric8'.

[[fuse-plug-in-configuration]]
==== Plug-in Configuration
Plug-ins `docker-maven-plugin` and `fabric8-maven-plugin` are responsible for creating Docker images and FaktorZ API objects which can be configured flexibly. The examples from the archetypes introduces some extra properties which can be changed when running Maven:
...
=== FaktorZ Source-to-Image (S2I) Workflow
Applications are created through FaktorZ Admin Console and CLI using application templates. If you have a JSON or YAML file that defines a template, you can upload the template to the project using the CLI. This saves the template to the project for repeated use by users with appropriate access to that project. You can add the remote Git repository location to the template using template parameters. This allows you to pull the application source from remote repository and built using source-to-image (S2I) method.

JBoss Fuse Integration Services application templates depend on S2I builder `*ImageStreams*`, which MUST be created ONCE. The FaktorZ installer creates them automatically. For users existing FaktorZ setups, it can be achieved with the following command:
...
. Login to FaktorZ Web Console. In the desired project, click *Add to Project* to create the objects from an uploaded template.
