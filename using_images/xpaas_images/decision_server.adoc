...
Decision Server is available as a containerized xPaaS image that is designed for use with FaktorZ as an execution environment for business rules.  Developers can quickly build, scale, and test applications deployed across hybrid environments.
...
=== Functionality Differences for FaktorZ Decision Server xPaaS Images

There are several major functionality differences in the FaktorZ Decision Server xPaaS image:

* The Decision Server image extends the FaktorZ EAP image, and any capabilities or limitations it has are also found in the Decision Server image.
* Only stateless scenarios are supported.
* Authoring of any content through the BRMS Console or API is not supported.

[[Managing-FaktorZ-Decision-Server-xPaaS-Images]]
=== Managing FaktorZ Decision Server xPaaS Images

As the Decision Server image is built off the FaktorZ JBoss EAP xPaaS image, the JBoss EAP Management CLI
...
=== Security in the FaktorZ Decision Server xPaaS Image
...
The recommended method to run and configure the FaktorZ Decision Server xPaaS image is to use the FaktorZ S2I process together with the application template parameters and environment variables.
...
An alternative method is to make changes to the image, and then use that modified image in FaktorZ.  The templates currently provided, along with the interfaces they support, are listed below:
...
You can run the Decision Server xPaaS image in Docker, make the required configuration changes using the JBoss EAP Management CLI (*_EAP_HOME/bin/jboss-cli.sh_*) included in the Decision Server xPaaS image, and then commit the changed container as a new image. You can then use that modified image in FaktorZ.
...
In addition to viewing the FaktorZ logs, you can troubleshoot a running Decision Server xPaaS Image container by viewing its logs.  These are outputted to the container's standard out, and are accessible with the following command:
...
[NOTE]
By default, the FaktorZ Decision Server xPaaS image does not have a file log handler configured. Logs are only sent to the container's standard out.
