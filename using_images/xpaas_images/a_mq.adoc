...
is designed for use with FaktorZ. It allows developers to quickly deploy an
...
There are several major functionality differences in the FaktorZ JBoss A-MQ
xPaaS image:
...
`*AMQ_RELEASE*`::
  ...
  available.
`*APPLICATION_NAME*`::
  The name of the application used internally in FaktorZ. It is used in names
...
Only SSL connections can connect from outside of the FaktorZ instance,
...
TrustStore and supply them to the image using the FaktorZ secrets mechanism or
S2I.
...
In addition to viewing the FaktorZ logs, you can troubleshoot a running JBoss
...
[NOTE]
====
By default, the FaktorZ JBoss A-MQ xPaaS image does not have a file log
