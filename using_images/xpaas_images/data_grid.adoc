...
Red Hat JBoss Data Grid is available as a containerized xPaaS image that is designed for use with FaktorZ.  This image provides an in-memory distributed database so that developers can quickly access large amounts of data in a hybrid environment.
...
=== Functionality Differences for FaktorZ JBoss Data Grid xPaaS Images

There are several major functionality differences in the FaktorZ JBoss Data Grid xPaaS image:

* The JBoss Data Grid Management Console is not available xref:Managing-FaktorZ-JBoss-Data-Grid-xPaaS-Images[to manage FaktorZ JBoss Data Grid xPaaS images].
...
[[jdg-clustering]]
=== Forming a Cluster using the FaktorZ JBoss Data Grid xPaaS Images
...
[IMPORTANT]
Only caches with an exposed REST endpoint will be accessible outside of FaktorZ.
...
The recommended method to run and configure the FaktorZ JBoss Data Grid xPaaS image is to use the FaktorZ S2I process together with the application template parameters and environment variables.
...
The JBoss Data Grid configuration file that FaktorZ uses inside the JBoss Data Grid xPaaS image is *_JDG_HOME/standalone/configuration/clustered-openshift.xml_*, and the JBoss Data Grid startup script is *_JDG_HOME/bin/openshift-launch.sh_*.

You can run the JBoss Data Grid xPaaS image in Docker, make the required configuration changes using the JBoss Data Grid Management CLI (*_JDG_HOME/bin/jboss-cli.sh_*), and then commit the changed container as a new image. You can then use that modified image in FaktorZ.

[IMPORTANT]
It is recommended that you do not replace the FaktorZ placeholders in the JBoss Data Grid xPaaS configuration file, as they are used to automatically configure services (such as messaging, datastores, HTTPS) during a container's deployment. These configuration values are intended to be set using environment variables.
...
| *_HOTROD_SERVICE_NAME_* | Name of the FaktorZ service used to expose HotRod externally. | Example: *_DATAGRID_APP_HOTROD_*
...
In addition to viewing the FaktorZ logs, you can troubleshoot a running JBoss Data Grid xPaaS Image container by viewing its logs. These are outputted to the container’s standard out, and are accessible with the following command:
...
[NOTE]
By default, the FaktorZ JBoss Data Grid xPaaS Image does not have a file log handler configured. Logs are only sent to the container's standard out.
