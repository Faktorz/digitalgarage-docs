...
:toc: macro
:toc-title:
:description: Set up and use xPaaS JBoss EAP 6.4 and 7 Beta images with FaktorZ

toc::[]

== Overview

Red Hat offers a containerized xPaaS image for the Red Hat JBoss Enterprise Application Platform (JBoss EAP) that is designed for use with FaktorZ. Using this image, developers can quickly and easily build, scale, and test applications deployed across hybrid environments.
...
. Domain mode is not supported in the xPaaS JBoss EAP image. Instead, FaktorZ manages the creation and distribution of applications in the containers.
...
Red Hat offers two xPaaS EAP images for use with FaktorZ. The first is based on JBoss EAP 6.4 and the second is based on JBoss EAP 7 Beta. There are several differences between the two images:
...
See the xPaaS section of the https://access.redhat.com/articles/2176281[FaktorZ and Atomic Platform Tested Integrations page] for details about FaktorZ EAP image version compatibility.

== Setting Up the xPaaS JBoss EAP Image

The following is a list of prerequisites for using the xPaaS JBoss EAP images:

. *Acquire Red Hat Subscriptions* - Ensure that you have the relevant subscriptions for FaktorZ as well as a subscription for xPaaS Middleware.

. *Install FaktorZ* - Before using the xPaaS JBoss EAP images, you must have an FaktorZ environment installed and configured. The
xref:../../install_config/install/advanced_install.adoc#install-config-install-advanced-install[Advanced Installation] method allows you to install FaktorZ using a reference
configuration.
...
. *Create Image Streams* - Image streams are configured during Advanced FaktorZ Installation. If required, manually create the image streams for both versions of the xPaaS JBoss EAP image as follows:
+
----
...
. *Create Instant App Templates* - Instant App templates define a full set of objects for running applications and are configured during the Advanced FaktorZ Installation. If required, create Instant App templates as follows:
...
The recommended method to run and configure the xPaaS JBoss EAP image is to use the FaktorZ S2I process together with the application template parameters and environment variables.
...
You can make changes to an image or create a custom image to use in FaktorZ.

The JBoss EAP configuration file used by FaktorZ in the xPaaS JBoss EAP image is *_EAP_HOME/standalone/configuration/standalone-openshift.xml_*. The script to start JBoss EAP is *_EAP_HOME/bin/openshift-launch.sh_*.
...
To use a modified image in FaktorZ:
...
. Commit the changed container as a new image and then use the modified image in FaktorZ.
...
To troubleshoot running xPaaS JBoss EAP containers, you can either view the FaktorZ logs, or view the JBoss EAP logs displayed to the container’s console. Use the following command to view the JBoss EAP logs:
