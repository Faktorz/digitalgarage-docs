...
The Apache Tomcat 7 and Apache Tomcat 8 components of Red Hat JBoss Web Server 3 are available as containerized xPaaS images that are designed for use with FaktorZ. Developers can use these images to quickly build, scale, and test Java web applications deployed across hybrid environments.
...
== Functionality Differences in the FaktorZ JBoss Web Server xPaaS Images

A major functionality difference compared to the regular release of JBoss Web Server is that there is no Apache HTTP Server in the FaktorZ JBoss Web Server xPaaS images. All load balancing in FaktorZ is handled by the FaktorZ router, so there is no need for a load-balancing Apache HTTP Server with mod_cluster or mod_jk connectors.
...
To run and configure the FaktorZ JBoss Web Server xPaaS images, use the FaktorZ S2I process with the application template parameters and environment variables.
...
In addition to viewing the FaktorZ logs, you can troubleshoot a running JBoss Web Server container by viewing the logs that are outputted to the container's console:
