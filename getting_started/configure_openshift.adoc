...
deploying or installing a production environment of FaktorZ.
...
. Restart FaktorZ before going forward.
+
----
...
[[deploy-router]]
== Deploy the FaktorZ Router

The FaktorZ router is the entry point for external network traffic destined
for FaktorZ services. It supports HTTP, HTTPS, and any TLS-enabled traffic
that uses SNI, which enables the router to send traffic to the correct service.

Without the router, FaktorZ services and pods are unable to communicate with
any resource outside of the FaktorZ instance.
...
The FaktorZ documentation contains detailed information on
...
to locally manage images. FaktorZ uses the *docker-registry* to store,
...
registry. The registry is just another image in the FaktorZ environment.
...
. Create the registry persistent volume in FaktorZ.
+
----
