...
InstantApp template. From there, the Digital Garage begins the build process and creates
a new deployment.
...
=======
. Make a code change that will be visible from within your application. For example: on line 229, change the title from `Welcome to your Ruby application on the Digital Garage` to `This is my Awesome the Digital Garage Application`, then save your changes.
>>>>>>> origin/master
