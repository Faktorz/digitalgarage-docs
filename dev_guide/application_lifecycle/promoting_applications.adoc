...
* The Jenkins image contains the FaktorZ Pipeline plug-in, which provides
...
leverage the steps provided by the FaktorZ Pipeline Plug-in.
...
The
link:https://github.com/openshift/jenkins/blob/master/2/contrib/openshift/configuration/jobs/FaktorZ%20Sample/config.xml[FaktorZ Sample] job defined in the link:https://github.com/openshift/jenkins[Jenkins
...
located in the
link:https://github.com/openshift/origin/blob/master/examples/jenkins/README.md[FaktorZ Origin source repository].


This sample includes:

* Use of *Jenkins as the CI/CD engine*.
* Use of the *FaktorZ Pipeline plug-in for Jenkins*. This plug-in provides a
...
link:https://github.com/openshift/origin/blob/master/examples/jenkins/application-template.json[FaktorZ
...
The following examines the various pieces of the FaktorZ Sample job in more
detail:

. link:https://github.com/openshift/jenkins/blob/master/2/contrib/openshift/configuration/jobs/FaktorZ%20Sample/config.xml#L15-L21[The first step] is the equivalent of an `oc scale dc frontend --replicas=0`
call. This step is intended to bring down any previous versions of the
application image that may be running.

. link:https://github.com/openshift/jenkins/blob/master/2/contrib/openshift/configuration/jobs/FaktorZ%20Sample/config.xml#L23-L29[The second step] is the equivalent of an `oc start-build frontend` call.

. link:https://github.com/openshift/jenkins/blob/master/2/contrib/openshift/configuration/jobs/FaktorZ%20Sample/config.xml#L31-L39[The third step] is the equivalent of an `oc deploy frontend --latest` or `oc rollout
latest dc/frontend` call.

. link:https://github.com/openshift/jenkins/blob/master/2/contrib/openshift/configuration/jobs/FaktorZ%20Sample/config.xml#L41-47[The fourth step] is the "test" for this sample. It ensures that the associated
...
can be added (if not via FaktorZ Pipepline plug-in steps, then via use of the
Jenkins Shell step to leverage OS-level commands and scripts to test your
application).

. link:https://github.com/openshift/jenkins/blob/master/2/contrib/openshift/configuration/jobs/FaktorZ%20Sample/config.xml#L49-L61[The fifth step] commences under that assumption that the testing of your application
...
. link:https://github.com/openshift/jenkins/blob/master/2/contrib/openshift/configuration/jobs/FaktorZ%20Sample/config.xml#L63-L73[The sixth and last step] is a verification step, where the plug-in confirms that
