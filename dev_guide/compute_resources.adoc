...
Opaque integer resources allow cluster operators to advertise new node-level
resources that would be otherwise unknown to the system. Users can consume these
resources in pod specifications, just like CPU and memory. The scheduler takes
care of the resource accounting so that no more than the available amount is
...
operator must advertise a per-node opaque resource on one or more nodes. Second,
users must request the opaque resource in pods.
...
fitness, there may be a short delay between patching the node capacity with a
new resource and the first pod that requests the resource to be scheduled on
that node.
====

The HTTP request below advertises five `foo` resources on node
`openshift-node-1`. Note that `~1` is the encoding for the character `/` in the
patch path. The operation path value in the JSON-Patch is interpreted as a
...
To consume opaque resources in pods, include the name of the opaque resource as
a key in the the `spec.containers[].resources.requests` map.
...
The pod below requests two CPUs and one `foo` (an opaque resource).
