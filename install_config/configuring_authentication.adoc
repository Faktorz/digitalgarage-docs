...
_https://<master>/oauth/authorize_ via an authenticating proxy. You can either proxy the
...
* <platform> = `linux`
* <architecture> = `amd64`
* <client> = `openshift` (or `kubernetes` depending on if you accessed the Kubernetes API in `/api` instead of the {product-title} API in `/oapi`)
* <git_commit> = The Git commit of the client version (for example, `f034127`)

the user agent will be:

----
oc/v1.3.0 (linux/amd64) openshift/f034127
----
