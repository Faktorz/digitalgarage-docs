...
== Overview
As an OpenShift cluster administrator, you can manage users and roles, as well
as the projects, templates, and available resources they manage themselves. For
now, this topic focuses on common operations a cluster administrator needs to
perform. You can:
...
* link:../admin_solutions/user_role_mgmt.html#control-monitor-resources[Monitor resource usage],
enabling you to check the amount of used resources, such as CPU usage.
...
You can place limits within your OpenShift cluster using link:../admin_guide/quota.html[ResourceQuotas]
and
link:../admin_guide/limits.html[LimitRanges],
...
*The use case above is just a basic example. Below is the same procedure, but with references to all the available options:*

. To create project limits or quotas, you must first write a .yaml file that describes the resource that you want to restrict:

* This *_LimitRange_* example explains all the
link:../admin_guide/limits.html#container-limits[container limits] and
...
+
include::admin_guide/quota.adoc[tag=admin_quota_object_counts]
+
...
----
+
The `-n` flag is optional. Use it to create the limit in a different project
than the one you currently are in.
...
link:../architecture/additional_concepts/admission_controllers.html[using an admission control plug-in]. This allows some users to create more projects than others, and you can define different values (or levels) for each label.

*Use Case: You want to define 3 different levels for your users which define how
*many projects they can request.*
...
controlled by the
link:../architecture/additional_concepts/authorization.html#roles[various types]
of
<<<<<<< HEAD
...
link:../admin_guide/manage_authorization_policy.html#managing-role-bindings[change the roles bound to these groups].
=======
...
You can define
link:../admin_guide/manage_authorization_policy.html[roles] (or permissions)
=======
...
To bind a
link:../architecture/additional_concepts/authorization.html#cluster-policy-and-local-policy[cluster role]
...
no restrictions are enforced in that namespace (i.e., any subject is allowed).
...
----
$ oc create -f - -n swoodilypoopers <<EOF
apiVersion: v1
...
----
<1> Match against user subjects. A RoleBindingRestriction spec must specify
exactly one of "userrestriction", "grouprestriction", or
"serviceaccountrestriction".
<2> Match any user with the name "callum" or "seb".
<3> Match any user that is in the "swoodilypoopers" group.
...
"callum" or "seb" will be allowed. Role bindings with subjects that are members
of the "swoodilypoopers" group or who match the specified label will also be
...
----
$ oadm policy add-role-to-user view ojgreen -n swoodilypoopers
Error from server: rolebindings "view" is forbidden: rolebindings to User "ojgreen" are not allowed in project "swoodilypoopers"
$ oadm policy add-role-to-user view callum seb -n swoodilypoopers
$ oc get rolebindings/view -n swoodilypoopers
NAME      ROLE      USERS        GROUPS    SERVICE ACCOUNTS   SUBJECTS
view      /view     callum, seb
----

The following example creates a role binding restriction that permits role
bindings with the group "swoodilypoopers" as the subject:

.Example Role Binding Restriction Matching Groups
----
$ oc create -f - -n wimblywomblys <<EOF
apiVersion: v1
kind: RoleBindingRestriction
metadata:
  name: match-groups
...
<2> Match any group with the name "wimblywomblys".
...
<2> Match any service account with the name "wimbler" in the namespace
"wimbledon".
<3> Match any service account with the name "wombler" in the same namespace as
the RoleBindingRestriction object (in this case, the "wimblywomblys" namespace).
<4> Match any service account in the "wimblewomble" namespace.
...
== How to Create a Cluster Administrator User
