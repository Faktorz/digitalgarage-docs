...
_contexts_, for use with the xref:index.adoc#cli-reference-index[FaktorZ CLI]. A context
consists of xref:../dev_guide/authentication.adoc#dev-guide-authentication[user
...
After xref:get_started_cli.adoc#basic-setup-and-login[logging in with the CLI] for the first time,
...
nickname *alice/openshift1.example.com:8443* uses an access token.
====
...
nickname *alice* that uses an access token:
