...
[NOTE]
====
Pruning images removes data from the Red Hat Container Registry. For this operation to work
properly, ensure your
xref:../install_config/install/docker_registry.adoc#docker-registry-configuration-reference-storage[registry
is configured] with `*storage:delete:enabled*` set to *true*.
====
>>>>>>> openshift/online
