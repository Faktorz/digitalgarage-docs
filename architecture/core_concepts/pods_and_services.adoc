...
[WARNING]
====
Bare pods that are not managed by a xref:deployments.adoc#replication-controllers[replication
...
IP.
<7> {product-title} defines a security context for containers which specifies whether they are allowed to run as privileged containers, run as a user of their choice, and more. The default
...
xref:service-externalip[external] to cluster, can be assigned to the service. These externalIP addresses can also be virtual IP addresses that provide
ifdef::openshift-enterprise,openshift-origin[]
xref:../../admin_guide/high_availability.adoc#admin-guide-high-availability[highly available]
endif::[]
ifdef::openshift-dedicated,openshift-online,digital-garage,atomic-registry[]
highly available
endif::[]
...
The link:https://kubernetes.io/docs/concepts/services-networking/service[Kubernetes
...
ifdef::openshift-dedicated,openshift-online,digital-garage[]
The externalIPs must be selected by the admin from the *ExternalIPNetworkCIDRs*
...
network blocks reserved for pod and service IPs. Only Digital Garage admins can create such Endpoint objects.
