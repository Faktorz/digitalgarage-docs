...
ifdef::openshift-origin,digital-garage,openshift-online,openshift-enterprise,openshift-dedicated[]
Digital Garage is based on OpenShift v3, a layered system designed to expose underlying Docker-formatted
container images and Kubernetes concepts as accurately as possible, with a focus
...
xref:additional_concepts/authentication.adoc[authenticate] users who present
credentials, and then xref:additional_concepts/authorization.adoc[authorize]
them based on their role. Both developers and administrators can be
authenticated via a number of means, primarily
xref:additional_concepts/other_api_objects.adoc#oauth-objects[OAuth tokens] and SSL
